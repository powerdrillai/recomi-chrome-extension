...
async function embedChatbot() {
  const recomiChatbotConfig = window.recomiChatbotConfig;
  if (!recomiChatbotConfig) {
    console.warn('Recomi AI Chatbot URL is empty or is not provided');
    return;
  }
  const openIcon = `<svg
            id="openIcon"
  ...
  function createIframe() {
    ...
    iframe.title = "recomi chatbot bubble window"
    iframe.id = 'recomi-chatbot-bubble-window'
    iframe.src = recomiChatbotConfig.chatbotUrl
  ...
  const targetButton = document.getElementById("recomi-chatbot-bubble-button");

  if (!targetButton) {
    // create button
    const containerDiv = document.createElement("div");
    containerDiv.id = 'recomi-chatbot-bubble-button';
    ...
    containerDiv.addEventListener('click', function () {
      const targetIframe = document.getElementById('recomi-chatbot-bubble-window');
      if (!targetIframe) {
        createIframe();
